...
static const struct bt_mesh_sensor_format bt_mesh_sensor_format_icm20948 = {
    .size = 2,  // 假设模拟的值是一个 16 位整数
	//.size = 18,  /* Total size in bytes */
    //.type = BT_MESH_SENSOR_TYPE_ACCELEROMETER,  /* 示例：你需要根据实际数据类型选择 */
    /* 其他格式字段，如编码方式等 */
};

/* Define one channel, referencing the above format */
static const struct bt_mesh_sensor_channel icm20948_channels[] = {
    {
        .format = &bt_mesh_sensor_format_icm20948,  /* 使用上面定义的格式 */
        //.unit = NULL,                               /* 根据需要设置单位 */
        //.encoding = BT_MESH_SENSOR_ENCODING_RAW,    /* 例如，可以是原始数据编码，具体取决于你的传感器类型 */
        //.name = "ICM20948 Sensor",                  /* 可选，传感器的名称 */
        /* 其他字段，如 "value_type", "sensor_value", 以及你可能需要的任何参数 */
    }
};

/* Our new custom sensor type with a single channel. */
static const struct bt_mesh_sensor_type bt_mesh_sensor_icm20948 = {
    .id = BT_MESH_PROP_ID_CUSTOM_ICM20948,   /* The vendor-specific property ID */
    .channel_count = ARRAY_SIZE(icm20948_channels),
    .channels = icm20948_channels,
};


// static int icm20948_get(struct bt_mesh_sensor_srv *srv,
//                         struct bt_mesh_sensor *sensor,
//                         struct bt_mesh_msg_ctx *ctx,
//                         struct bt_mesh_sensor_value *rsp)
// {
//     printk("Fetching data for ICM-20948 sensor via Mesh framework.\n");

//     /* 模拟传感器的加速度和角速度数据 */
//     float accel_x = 1.23f;  // 模拟 X 轴加速度（单位 g）
//     float accel_y = -0.56f; // 模拟 Y 轴加速度（单位 g）
//     float accel_z = 9.81f;  // 模拟 Z 轴加速度（单位 g）
//     float gyro_x = 45.6f;   // 模拟 X 轴角速度（单位 °/s）
//     float gyro_y = -30.2f;  // 模拟 Y 轴角速度（单位 °/s）
//     float gyro_z = 10.0f;   // 模拟 Z 轴角速度（单位 °/s）
...
//     // 加速度和角速度分量逐一转换
//     err = bt_mesh_sensor_value_from_float(sensor->type->channels[0].format, accel_x, &rsp[index++]);
//     if (err && err != -ERANGE) return err;

//     err = bt_mesh_sensor_value_from_float(sensor->type->channels[0].format, accel_y, &rsp[index++]);
//     if (err && err != -ERANGE) return err;

//     err = bt_mesh_sensor_value_from_float(sensor->type->channels[0].format, accel_z, &rsp[index++]);
//     if (err && err != -ERANGE) return err;

//     err = bt_mesh_sensor_value_from_float(sensor->type->channels[0].format, gyro_x, &rsp[index++]);
//     if (err && err != -ERANGE) return err;

//     err = bt_mesh_sensor_value_from_float(sensor->type->channels[0].format, gyro_y, &rsp[index++]);
//     if (err && err != -ERANGE) return err;

//     err = bt_mesh_sensor_value_from_float(sensor->type->channels[0].format, gyro_z, &rsp[index++]);
//     if (err && err != -ERANGE) return err;

//     printk("ICM-20948 data fetched successfully via Mesh.\n");
//     return 0;
// }
                        ...
                        struct bt_mesh_sensor_value *rsp)
{
    ...
    /* 模拟 "People Counting" 数据 */
    uint16_t people_count = 42;  // 模拟的读数

    /* 转换为 Bluetooth Mesh 格式 */
    int err = bt_mesh_sensor_value_from_float(sensor->type->channels[0].format, people_count, rsp);
    if (err && err != -ERANGE) {
        printk("Error encoding ICM-20948 data (%d)\n", err);
        return err;
    }

    printk("ICM-20948 data fetched successfully: People Counting = %u\n", people_count);
    return 0;
}
...
static const struct bt_mesh_sensor_descriptor icm20948_descriptor = {
    .sampling_type = BT_MESH_SENSOR_SAMPLING_INSTANTANEOUS, // 即时采样
    .period = 0, // 不强制设置发布周期
    .update_interval = 0, // 不强制设置更新间隔
};


static struct bt_mesh_sensor icm20948_sensor = {
    .type = &bt_mesh_sensor_icm20948, // 定义 ICM-20948 的自定义类型
    .get = icm20948_get,              // Mesh 框架调用此函数获取数据
    .descriptor = &icm20948_descriptor, // 描述符定义采样类型等属性
};


static struct bt_mesh_sensor *const icm20948_sensors[] = {
    &icm20948_sensor,
};

static struct bt_mesh_sensor_srv icm20948_sensor_srv =
    BT_MESH_SENSOR_SRV_INIT(icm20948_sensors, ARRAY_SIZE(icm20948_sensors));
...
static struct bt_mesh_elem elements[] = {
	...
	BT_MESH_ELEM(4,
		     ...
		     BT_MESH_MODEL_NONE),
	// BT_MESH_ELEM(4,
    //     	BT_MESH_MODEL_LIST(BT_MESH_MODEL_SENSOR_SRV(&icm20948_sensor_srv)),
    //     	BT_MESH_MODEL_NONE),
	BT_MESH_ELEM(5,
...
if (icm20948_dev == NULL) {
    printk("ICM-20948 device is simulated and not initialized.\n");
}
